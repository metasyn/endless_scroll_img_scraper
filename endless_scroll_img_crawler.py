...
def imgGreb():
	...
	#browser.get(myurl)
	browser.get("http://uuuu----uuuu.tumblr.com/")
	...
	while(html != temp_html):
		...
		if(html == temp_html and check_at_end == False):
			temp_html = ''
			check_at_end = True
		# if we just double checked that we were at the bottom and we are not, reset check_at_end to False
		#elif(check_at_end == True and html != temp_html):
		#	check_at_end == False
